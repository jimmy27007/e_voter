      ...
      if (resJson.error_state === true) {
        ...
        setTimeout(() => {
          ...
          window.location.href =
            "http://" + window.location.host + "/evoter/vote/";
        }, 2000);
      }
...
function register(a) {
  a.addEventListener("click", () => {
    window.location.href =
      "http://" + window.location.host + "/evoter/registration/";
  });
}
  ...
  fetch("http://" + location.hostname + "/evoterphp/login/verifylogin.php", {
    ...
    res.json().then((data) => {
      switch (data.status) {
        case "email_sent":
          notify(
            "Please check your email <h1 style='color:green;'><u>" + data.message + "</u></h1> for verification code.<br> <B><U>NOTE:</U></B> If you dont see it, please refresh or check the spam filter."
          );
          break;
        case "email_not_sent":
...
var gen;
var details;
function validate_registration_details(
  name,
  name1,
  id,
  age,
  ...
  submit.addEventListener("click", () => {
    ...
    if (details.name.trim() == "") {
      ...
      submitRegistrationData(
        detailsJson,
        "http://" + location.hostname + "/evoterphp/registration/",
        `${details.name}, Please wait for us to handle the data.`
      );
    }
  });
}
...
function validate_login_details(
  ...
  submit.addEventListener("click", () => {
    ...
    start_loading(submit,"Loging in","Log in",true,2);
      ...
      submitLoginData(
        detailsJson,
        "http://" + window.location.hostname + "/evoterphp/login/",
        `Welcome back to your account ${details.voters_id}. Vote a leader of your choice.`
      );
...
function render_counties(a) {
  fetch("/evoter/json/counties.json", {
    ...
    .then((data) => {
      ...
      counties_data.sort();
      for (const dat of counties_data) {
        a.innerHTML += `<option>${dat}</option>`;
      }
    });
}
function resetPassword() {
  window.location.href =
    "http://" + window.location.hostname + "/evoter/login/resetpin";
}
